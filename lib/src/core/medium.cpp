#include "specula/core/medium.hpp"
...
static MeasuredSS subsurface_parameter_table[] = {
    {
        "Apple",
        {2.29, 2.39, 1.97},
        {0.0030, 0.0034, 0.046},
...
specula::PhaseFunction::~PhaseFunction() {}

bool specula::get_medium_scattering_properties(const std::string &name,
                                               Spectrum *sigma_a,
                                               Spectrum *sigma_s) {
  for (MeasuredSS &mss : subsurface_parameter_table) {
    if (name == mss.name) {
      *sigma_a = Spectrum::from_rgb(mss.sigma_a);
      *sigma_s = Spectrum::from_rgb(mss.sigma_prime_s);
      return true;
    }
  }
  return false;
}
specula::Float specula::HenyeyGreenstein::sample_p(const Vector3f &wo,
                                                   Vector3f *wi,
  ...
  if (std::abs(g) < 1e-3) {
    ...
    Float sqrt_term = (1 - g * g) / (1 - g + 2 * g * u[0]);
    cos_theta = (1 + g * g - sqrt_term * sqrt_term) / (2 * g);
  }
...
specula::Float specula::HenyeyGreenstein::p(const Vector3f &wo,
                                            const Vector3f &wi) const {
  PROF_FUNC_ARGS(wo, wi);
