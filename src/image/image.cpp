...
specula::Image::Image(const std::array<std::size_t, 2> &resolution)
    : resolution_(resolution), pixels_(resolution[0] * resolution[1],
                                       std::array<double, 3>{0.0, 0.0, 0.0}) {}

bool specula::Image::write(const std::string &filename) const {
  if (filename.length() >= 4 &&
      0 == filename.compare(filename.length() - 4, 4, ".png")) {
    return write_png(filename);
  } else if (filename.length() >= 5 &&
             0 == filename.compare(filename.length() - 5, 5, ".jpeg")) {
    return write_jpeg(filename);
  } else if (filename.length() >= 4 &&
             0 == filename.compare(filename.length() - 4, 4, ".bmp")) {
    return write_bmp(filename);
  }
  return false;
}

bool specula::Image::write_png(const std::string &filename) const {
...
bool specula::Image::write_jpeg(const std::string& filename) const {
  linfo("Writing image to \"%s\"", filename.c_str());
  lerror("JPEG format is not yet supported");
  return false;
};
bool specula::Image::write_bmp(const std::string& filename) const {
  linfo("Writing image to \"%s\"", filename.c_str());
  lerror("BMP format is not yet supported");
  return false;
};
