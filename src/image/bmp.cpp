  ...
  for (std::size_t i = 0; i < resolution.w; ++i) {
    for (std::size_t j = 0; j < resolution.h; ++j) {
      ...
      img[(x + y * resolution.w) * 3 + 2] =
          static_cast<uint8_t>(buffer[i + j * resolution.w].r * 0xff);
      img[(x + y * resolution.w) * 3 + 1] =
          static_cast<uint8_t>(buffer[i + j * resolution.w].g * 0xff);
      img[(x + y * resolution.w) * 3 + 0] =
          static_cast<uint8_t>(buffer[i + j * resolution.w].b * 0xff);
    }
  }
