...
specula::Sphere::Sphere(const float &r) : Primative(), radius_(r) {}
specula::Sphere::Sphere(const float &r, const std::array<double, 3> &c)
    : Primative(c), radius_(r) {}
specula::Sphere::Sphere(const float &r, const glm::vec3 &p,
...
float specula::Sphere::intersect(const glm::vec3 &o, const glm::vec3 &d) const {
  ...
  // ldebug("%s->%s", glm::to_string(origin).c_str(),
