...
specula::CappedCone::CappedCone(const float &h, const float &r1,
  ...
  this->source_ =
      fmt::format<512>("float2 q = (float2)(length(p.xz), p.y); float2 k1 = "
                  "(float2)({{%1$s-capped-cone.r2}},{{%1$s-capped-cone.height}}"
                  "); float2 k2 = "
                  "(float2)({{%1$s-capped-cone.r2}}-{{%1$s-capped-cone.r1}},2."
                  "0f*{{%1$s-capped-cone.height}}); float2 ca = "
                  "(float2)(q.x-min(q.x,(q.y<0.0f)?{{%1$s-capped-cone.r1}}:{{%"
                  "1$s-capped-cone.r2}}), "
                  "fabs(q.y)-{{%1$s-capped-cone.height}}); float2 cb = "
                  "q-k1+k2*clamp(dot(k1-q,k2)/dot(k2,k2),0.0f,1.0f); float s "
                  "= (cb.x < 0.0f && ca.y < 0.0f) ? -1.0f : 1.0f; return "
