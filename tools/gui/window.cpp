...
void gui::Window::open() {
  if(open_callback) open_callback(shared_from_this());
}
void gui::Window::close() {
  if(delete_on_close) {
    gui::remove_window(shared_from_this());
    deleted();
  }else if(close_callback) close_callback(shared_from_this());
}
void gui::Window::deleted() {
  if(delete_callback) delete_callback(shared_from_this());
}
