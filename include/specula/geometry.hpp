/**
 * @file geometry.hpp
 * @brief Geometric classes (Points, Vectors, Rays, etc.)
 *
 * Almost all graphcis programs are build upon geometric classes. These classes
 * represent mathematical constructs like points, vectors, and rays. Because
 * these classes are used through the system, good abstractions and efficient
...
public:
  Vector2() : x(0), y(0) {}
  Vector2(T x, T y) : x(x), y(y) { SPECULA_ASSERT(!has_NaNs()); }

  inline bool has_NaNs() const { return std::isnan(x) || std::isnan(y); }
...
public:
  Vector3() : x(0), y(0), z(0) {}
  Vector3(T x, T y, T z) : x(x), y(y), z(z) { SPECULA_ASSERT(!has_NaNs()); }
