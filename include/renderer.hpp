...
bool render_frame(const fs::path &file,
                  ...
                  const unsigned2 &res, const std::array<float3, 3> &camera,
                  const bool &denoise);
std::tuple<std::size_t, std::vector<std::vector<float3>>,
           std::vector<std::vector<float3>>,
           std::vector<std::vector<float3>>, std::vector<std::vector<float>>>
render_tile(const std::size_t &tile_id, const unsigned4 &tile,
            const unsigned2 &res,
            const std::vector<std::shared_ptr<object::Object>> &objs,
            const std::size_t &spp, const float &fov,
            const std::size_t &min_bounce, const float &filmz,
            const glm::mat4 &view);
std::tuple<float3, float3, float3, float>
ray_march(const float3 &o, const float3 &d,
          const std::vector<std::shared_ptr<object::Object>> &objs,
...
ray_intersect(const float3 &o, const float3 &d,
              const std::vector<std::shared_ptr<object::Object>> &objs,
